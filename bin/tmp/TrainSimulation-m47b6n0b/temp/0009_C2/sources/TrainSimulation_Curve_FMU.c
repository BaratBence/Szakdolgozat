...
#define MODEL_GUID "{dca43f20-c975-4751-921d-94bee49e2fc9}"
...
void setDefaultStartValues(ModelInstance *comp) {
  ...
  comp->fmuData->modelData->realParameterData[4].attribute.start = 152.0;
...
static const int realAliasIndexes[10] = {
...
fmi2Real getReal(ModelInstance* comp, const fmi2ValueReference vr) {
  ...
  if (vr < 39) {
    return comp->fmuData->simulationInfo->realParameter[vr-32];
  }
  if (vr < 49) {
    int ix = realAliasIndexes[vr-39];
...
fmi2Status setReal(ModelInstance* comp, const fmi2ValueReference vr, const fmi2Real value) {
  ...
  if (vr < 39) {
    comp->fmuData->simulationInfo->realParameter[vr-32] = value;
    return fmi2OK;
  }
  if (vr < 49) {
    int ix = realAliasIndexes[vr-39];
