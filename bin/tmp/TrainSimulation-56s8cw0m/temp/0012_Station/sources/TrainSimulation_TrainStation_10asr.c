...
*/
void TrainSimulation_TrainStation_eqFunction_77(DATA *data, threadData_t *threadData)
{
  TRACE_PUSH
  ...
  if(!tmp4)
  {
    ...
    tmp1 = LessEq(data->localData[0]->integerVars[1] /* state DISCRETE */,4);
    if(!(tmp0 && tmp1))
    {
      ...
      {
        FILE_INFO info = {"E:/Szakdoga/TrainSimulation.mo",660,5,660,24,0};
...
OMC_DISABLE_OPT
int TrainSimulation_TrainStation_checkForAsserts(DATA *data, threadData_t *threadData)
{
  TRACE_PUSH

  TrainSimulation_TrainStation_eqFunction_77(data, threadData);
