...
#define MODEL_GUID "{1bd7b20b-4ada-4cdb-941b-39a4ec260400}"
...
void setDefaultStartValues(ModelInstance *comp) {
  ...
  comp->fmuData->modelData->realVarsData[13].attribute.start = 4.74;
  ...
  comp->fmuData->modelData->realParameterData[0].attribute.start = 13.14;
  ...
  comp->fmuData->modelData->realParameterData[3].attribute.start = 4.74;
  comp->fmuData->modelData->realParameterData[4].attribute.start = 8.76;
  comp->fmuData->modelData->realParameterData[5].attribute.start = 16.7;
...
fmi2Real getReal(ModelInstance* comp, const fmi2ValueReference vr) {
  ...
  if (vr < 24) {
    return comp->fmuData->simulationInfo->realParameter[vr-18];
  }
  if (vr < 26) {
    int ix = realAliasIndexes[vr-24];
...
fmi2Status setReal(ModelInstance* comp, const fmi2ValueReference vr, const fmi2Real value) {
  ...
  if (vr < 24) {
    comp->fmuData->simulationInfo->realParameter[vr-18] = value;
    return fmi2OK;
  }
  if (vr < 26) {
    int ix = realAliasIndexes[vr-24];
