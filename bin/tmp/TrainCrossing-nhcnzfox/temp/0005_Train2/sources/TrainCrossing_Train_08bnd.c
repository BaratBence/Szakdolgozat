...
*/
OMC_DISABLE_OPT
static void TrainCrossing_Train_eqFunction_8(DATA *data, threadData_t *threadData)
{
  TRACE_PUSH
  const int equationIndexes[2] = {1,8};
  data->modelData->realVarsData[0].attribute /* distance STATE(1,maxSpeed) */.start = data->simulationInfo->realParameter[0] /* distanceStart PARAM */;
...
int TrainCrossing_Train_updateBoundVariableAttributes(DATA *data, threadData_t *threadData)
{
  ...
  TrainCrossing_Train_eqFunction_8(data, threadData);
  if (ACTIVE_STREAM(LOG_INIT)) messageClose(LOG_INIT);
