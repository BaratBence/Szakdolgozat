...
#define MODEL_GUID "{a1e11616-f71d-4c63-9a42-a955b59735c5}"
...
fmi2Real getReal(ModelInstance* comp, const fmi2ValueReference vr) {
  ...
  if (vr < 5) {
    return comp->fmuData->simulationInfo->realParameter[vr-2];
  }
  if (vr < 6) {
    int ix = realAliasIndexes[vr-5];
...
fmi2Status setReal(ModelInstance* comp, const fmi2ValueReference vr, const fmi2Real value) {
  ...
  if (vr < 5) {
    comp->fmuData->simulationInfo->realParameter[vr-2] = value;
    return fmi2OK;
  }
  if (vr < 6) {
    int ix = realAliasIndexes[vr-5];
