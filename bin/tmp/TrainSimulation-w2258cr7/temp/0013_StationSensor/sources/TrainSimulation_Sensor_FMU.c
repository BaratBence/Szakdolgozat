...
#define MODEL_IDENTIFIER TrainSimulation_Sensor
#define MODEL_GUID "{30c7124c-152e-42c3-ab2d-fdc76b5beb93}"
...
extern void TrainSimulation_Sensor_setupDataStruc(DATA *data, threadData_t *threadData);
#define fmu2_model_interface_setupDataStruc TrainSimulation_Sensor_setupDataStruc
...
void setDefaultStartValues(ModelInstance *comp) {
  ...
  comp->fmuData->modelData->realParameterData[2].attribute.start = 100.0;
  comp->fmuData->modelData->realParameterData[3].attribute.start = 500.0;
...
fmi2Real getReal(ModelInstance* comp, const fmi2ValueReference vr) {
  ...
  if (vr < 29) {
    return comp->fmuData->simulationInfo->realParameter[vr-25];
  }
  if (vr < 30) {
    int ix = realAliasIndexes[vr-29];
...
fmi2Status setReal(ModelInstance* comp, const fmi2ValueReference vr, const fmi2Real value) {
  ...
  if (vr < 29) {
    comp->fmuData->simulationInfo->realParameter[vr-25] = value;
    return fmi2OK;
  }
  if (vr < 30) {
    int ix = realAliasIndexes[vr-29];
